...
export default function VideoStreaming({ navigation, route }) {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://172.20.10.9:3000/videoStreaming`);
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        for (const video of response.data) {
          const res = await axios.get('http://172.20.10.9:3000/is-like', {
  ...
  const toggleLike = async (idPost) => {
    try {
      ...
      const url = isCurrentlyLiked
        ? `http://172.20.10.9:3000/unlike`
        : `http://172.20.10.9:3000/like`;
  ...
  const checkIsLike = async (idPost, idUser) => {
    try {
      const response = await axios.get('http://172.20.10.9:3000/is-like', {
  ...
  const updateCurrentVideoData = async (idPost) => {
    try {
      const [likeResponse, commentResponse] = await Promise.all([
        axios.get(`http://172.20.10.9:3000/LikeCount?id=${idPost}`),
        axios.get(`http://172.20.10.9:3000/commentCount?id=${idPost}`),
      ]);
  ...
  const insertComment = async (idUser, idPost, text) => {
    try {
      const response = await axios.post('http://172.20.10.9:3000/insertComment', {
        idUser,
        idPost,
        text,
      });
