...
const MyVideos = ({id}) => {
    ...
    const fetchData = async (id) => {
      try {
        const response = await axios.get(`http://172.20.10.9:3000/profilevideos?id=${id}`);
...
export default function App({ navigation, route }) {
    ...
    const fetchData = async (id) => {
        try {
          const response = await axios.get(`http://172.20.10.9:3000/follow?id=${id}`);
          if (Array.isArray(response.data) && response.data.length > 0) {
  ...
  const checkIsFollowing = async (idFollowing, idFollowed) => {
      try {
          const response = await axios.get('http://172.20.10.9:3000/is-following', {
              params: {
  ...
  const follow = async (idFollowing, idFollowed) => {
    try {
        const response = await axios.post('http://172.20.10.9:3000/follow', {
...
const unfollow = async (idFollowing, idFollowed) => {
    try {
        const response = await axios.delete('http://172.20.10.9:3000/unfollow', {
            idFollowing: idFollowing,
            idFollowed: idFollowed,
        });
